export function createRhythmGame({root, chart, playNote, onProgress=()=>{}, onModeMessage=()=>{}}){
  ...
  const hitLine=root.querySelector('.rhythm-line');
  ...
  let accompIndex=0,accompEnabled=accompaniment.length>0,lastBeatIndex=-1;
  ...
  function frame(now){
    ...
    const beatFloat=(t-(chart.beatOrigin||0))*((chart.bpm||90)/60);
    const beatIndex=Math.floor(beatFloat+1e-6);
    if(beatFloat>=0&&beatIndex!==lastBeatIndex){
      lastBeatIndex=beatIndex;
      const downbeat=beatIndex%4===0;
      if(hitLine)hitLine.animate(
        [{filter:'brightness(1)',opacity:.78},{filter:downbeat?'brightness(1.65)':'brightness(1.3)',opacity:1},{filter:'brightness(1)',opacity:.86}],
        {duration:downbeat?210:150,easing:'ease-out'}
      );
    }
  ...
  function start(){
    ...
    startAt=performance.now()+PREROLL*1000;pausedTotal=0;paused=false;running=true;accompIndex=0;lastBeatIndex=-1;startBtn.textContent='重新开始';pauseBtn.textContent='暂停';
  ...
  function stop(clear=true){
    ...
    laneEls.forEach(el=>el.classList.remove('armed'));padEls.forEach(el=>el.classList.remove('armed'));accompIndex=0;lastBeatIndex=-1;
